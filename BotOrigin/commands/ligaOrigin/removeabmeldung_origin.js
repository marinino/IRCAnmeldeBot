...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('removeabmeldung_origin')
    ...
    async execute(client, interaction, command){
        ...
        if(!interaction.member.roles.cache.has(CurrentSeason.seasonData.getRennleiterRolleID()) &&
            ...
            console.log(`Der removeabmeldungOrigin Command wurde von ${interaction.user.username} verwendet -- ${date}`)
        }

        const userToRemoveWithdraw = interaction.options.getUser('fahrer');

        if(!(CurrentSeason.seasonData.getWithdrawnDriversPerCommandLigaOrigin().includes(userToRemoveWithdraw.id))){
            ...
            if(!(client.guilds.cache.get(CurrentSeason.seasonData.getDiscordID()).members.cache.get(userToRemoveWithdraw.id).roles.cache.has(CurrentSeason.seasonData.getStammfahrerRolleIDLigaOrigin()))){
            ...
            collectorConfirm.on('collect', async (reaction, user) => {
                ...
                if(reaction.emoji.name == CurrentSeason.seasonData.getAnmeldeEmoji()){
                    ...
                    let tempArray = CurrentSeason.seasonData.getWithdrawnDriversPerCommandLigaOrigin();
                    tempArray.splice(tempArray.indexOf(userToRemoveWithdraw.id), 1);
                    CurrentSeason.seasonData.setWithdrawnDriversPerCommandLigaOriginR(tempArray); 

                    let date = new Date().toLocaleString();
                    console.log(`removeabmeldungOrigin wurde gestartet und erfolgreich durchgeführt für ${userToRemoveWithdraw.username} -- ${date}`)
                    // Deletes message
                    ...
                    await interaction.channel.send(`Der Vorgang wurde erfolgreich abgebrochen`).then(() => {
                        let date = new Date().toLocaleString();
                        console.log(`removeabmeldungOrigin wurde gestartet und abgebrochen -- ${date}`)
                    });
                } else {
                    confirmMessage.reply('Es wurde mit dem falschen Emoji reagiert').then(() => {
                      let date = new Date().toLocaleString();
                      console.log(`removeabmeldungOrigin wurde gestartet aber es wurde mit dem falschen Emoji `+
