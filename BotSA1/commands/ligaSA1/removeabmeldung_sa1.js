...
module.exports = {
    ...
    async execute(client, interaction, command){
        ...
        if(!interaction.member.roles.cache.has(CurrentSeason.seasonData.getRennleiterRolleID()) &&
            ...
            console.log(`Der removeabmeldungSA1 Command wurde von ${interaction.user.username} verwendet -- ${date}`)
        }

        const userToRemoveWithdraw = interaction.options.getUser('fahrer');

        if(!(CurrentSeason.seasonData.getWithdrawnDriversPerCommandLigaSA1().includes(userToRemoveWithdraw.id))){
            ...
            if(!(client.guilds.cache.get(CurrentSeason.seasonData.getDiscordID()).members.cache.get(userToRemoveWithdraw.id).roles.cache.has(CurrentSeason.seasonData.getStammfahrerRolleIDLigaSA1()))){
            ...
            collectorConfirm.on('collect', async (reaction, user) => {
                ...
                if(reaction.emoji.name == CurrentSeason.seasonData.getAnmeldeEmoji()){
                    ...
                    let tempArray = CurrentSeason.seasonData.getWithdrawnDriversPerCommandLigaSA1();
                    tempArray.splice(tempArray.indexOf(userToRemoveWithdraw.id), 1);
                    CurrentSeason.seasonData.setWithdrawnDriversPerCommandLigaSA1(tempArray); 

                    let date = new Date().toLocaleString();
                    console.log(`removeabmeldungSA1 wurde gestartet und erfolgreich durchgeführt für ${userToRemoveWithdraw.username} -- ${date}`)
                    // Deletes message
                    ...
                    await interaction.channel.send(`Der Vorgang wurde erfolgreich abgebrochen`).then(() => {
                        let date = new Date().toLocaleString();
                        console.log(`removeabmeldungSA1 wurde gestartet und abgebrochen -- ${date}`)
                    });
                } else {
                    confirmMessage.reply('Es wurde mit dem falschen Emoji reagiert').then(() => {
                      let date = new Date().toLocaleString();
                      console.log(`removeabmeldungSA1 wurde gestartet aber es wurde mit dem falschen Emoji `+
