...
var confirmedSubsMadeLiga1 = new Map();
var confirmedSubsMadeInvLiga1 = new Map();
var flag = null;
...
const wartelisteRolleIDLiga1 = '869931101970378784';
...
var reservesActivatedLiga1 = null;
...
async function removeFromCurrentLineup(user, client, message, freeCar){
  if(user.roles.cache.has(stammfahrerRolleIDLiga1)){
    ...
    confirmedSubsMadeLiga1.delete(user.id);
    confirmedSubsMadeInvLiga1.delete(teamID);
...
async function changeLineupTripleChange(client, message, subDriverID, carToTake, mainTeamIDString, driverToStart){
  ...
  if(currentLineupLiga1.get(mainTeamNameString)[0] == subDriverID){
    positionOfSubDriver = 0;
  }else if(currentLineupLiga1.get(mainTeamNameString)[1] == subDriverID){
  ...
  currentLineupLiga1.get(mainTeamNameString)[positionOfSubDriver] = driverToStart;
...
function regularDriverBackCockpitFree(client, message, driverToStart, carToTake){
  ...
  if(currentLineupLiga1.get(teamNameMainDriver)[0] == 'nicht besetzt'){
    positionOfEmptySpot = 0;
  }else if(currentLineupLiga1.get(teamNameMainDriver)[1] == 'nicht besetzt'){
  ...
  currentLineupLiga1.get(teamNameMainDriver)[positionOfEmptySpot] = driverToStart;
  freeCarsLiga1.unshift(carToTake);
  freeCarsLiga1.splice(freeCarsLiga1.indexOf(client.guilds.cache.get(discordID).roles.cache.find(role => role.name == teamNameMainDriver).id),1);
...
async function checkSubCanBeMade(client, message, fromForceRemove, positionForForce, driverForForce, carForForce){
  if(freeCarsLiga1.length > 0 && ((subPersonListLiga1.length + subPersonListReinstatedDriversLiga1.length) > 0)){
    ...
    if(withdrawnDriversLiga1.length > 0 && !fromForceRemove &&
      ...
      currentLineupLiga1.get(findMainTeamString(client.guilds.cache.get(discordID).members.cache.get(driverToStart)))[0] != 'nicht besetzt' && 
      currentLineupLiga1.get(findMainTeamString(client.guilds.cache.get(discordID).members.cache.get(driverToStart)))[1] != 'nicht besetzt' &&
      ...
      let subDriverID = confirmedSubsMadeInvLiga1.get(mainTeamIDString);

      let subDriverToReplace = currentLineupLiga1.get(mainTeamNameString).indexOf(subDriverID);
      currentLineupLiga1.get(mainTeamNameString)[subDriverToReplace] = driverToStart;

      confirmedSubsMadeLiga1.delete(subDriverID);
      confirmedSubsMadeInvLiga1.delete(carToTake);

      confirmedSubsMadeLiga1.set(subDriverID.toString(), carToTake);
      confirmedSubsMadeInvLiga1.set(carToTake.toString(), subDriverID);
      ...
      confirmedSubsMadeLiga1.set(driverToStart.toString(), carToTake);
      confirmedSubsMadeInvLiga1.set(carToTake.toString(), driverToStart);
    ...
    } else if(!fromForceRemove && withdrawnDriversLiga1 > 0 && client.guilds.cache.get(discordID).members.cache.get(driverToStart) == 
              client.guilds.cache.get(discordID).members.cache.get(withdrawnDriversLiga1[withdrawnDriversLiga1.length - 1]) &&
              checkIfCarIsFree(client, carToTake)){
    ...
    } else if(client.guilds.cache.get(discordID).members.cache.get(driverToStart).roles.cache.has(stammfahrerRolleIDLiga1) && !fromForceRemove &&
      (currentLineupLiga1.get(findMainTeamString(client.guilds.cache.get(discordID).members.cache.get(driverToStart)))[0] != 'nicht besetzt' ||
      currentLineupLiga1.get(findMainTeamString(client.guilds.cache.get(discordID).members.cache.get(driverToStart)))[1] != 'nicht besetzt' )){
    ...
    } else if((client.guilds.cache.get(discordID).members.cache.get(driverToStart).roles.cache.has(stammfahrerRolleIDLiga1) ||
          client.guilds.cache.get(discordID).members.cache.get(driverToStart).roles.cache.has(ersatzfahrerRolleIDLiga1) ||
          client.guilds.cache.get(discordID).members.cache.get(driverToStart).roles.cache.has(wartelisteRolleIDLiga1))
          && withdrawnDriversLiga1.length == 0 && !fromForceRemove){
 ...
 */
 function initVariables(flag){
  ...
  confirmedSubsMadeLiga1 = new Map();
  confirmedSubsMadeInvLiga1 = new Map();
  msgLineupLiga1 = null;
  reservesActivatedLiga1 = false;
  return flag;
}
...
module.exports = {
    name: 'start1',
    aliases: ['anmelden1', 'abmelden1', 'removeabmeldung1', 'removeanmeldung1', 'activatereserves1', 'forceremove1',
    ...
    async execute(client, message, cmd, args, Discord){
        ...
        if(!(args[0])){
          ...
          collectorSubIn.on('collect', async (reaction, user) => {
            ...
            if(reaction.message.channel.id == anmeldeChannelIDLiga1){
              ...
              } else if (reaction.emoji.name === anmeldenEmoji && !(driversSubInPerCommandLiga1.includes(user.id)) &&
                          (reaction.message.guild.members.cache.get(user.id).roles.cache.has(wartelisteRolleIDLiga1)) &&
                          reservesActivatedLiga1){
                subDriverIn(client, reaction.message.guild.members.cache.get(user.id), message);                
              } else if (reaction.emoji.name === anmeldenEmoji && !(driversSubInPerCommandLiga1.includes(user.id)) &&
                        (reaction.message.guild.members.cache.get(user.id).roles.cache.has(wartelisteRolleIDLiga1)) &&
                        !(reservesActivatedLiga1)){
          ...
          collectorSubIn.on('remove', async (reaction, user) => {
            ...
            if(reaction.message.channel.id == anmeldeChannelIDLiga1){
              if(reaction.message.guild.members.cache.get(user.id).roles.cache.has(ersatzfahrerRolleIDLiga1) ||
                (reaction.message.guild.members.cache.get(user.id).roles.cache.has(wartelisteRolleIDLiga1) && reservesActivatedLiga1)){
              ...
              } else if((reaction.message.guild.members.cache.get(user.id).roles.cache.has(wartelisteRolleIDLiga1) && !reservesActivatedLiga1)){
                let date = new Date().toLocaleString();
      ...
      if(cmd == 'activatereserves1' && message.member.roles.cache.has(rennleiterRolleID)){

        if(reservesActivatedLiga1){
          ...
          collectorConfirm.on('collect', async (reaction, user) => {
            ...
            if(reaction.emoji.name == anmeldenEmoji){
              ...
              reservesActivatedLiga1 = true;
              await client.channels.cache.get(anmeldeChannelIDLiga1).send(`Die <@&${wartelisteRolleIDLiga1}> kann sich ab jetzt anmelden`);
