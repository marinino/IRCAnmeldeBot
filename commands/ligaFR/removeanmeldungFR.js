...
module.exports = {
    ...
    async execute(client, interaction, command){
        ...
        if(!interaction.member.roles.cache.has(CurrentSeason.seasonData.getRennleiterRolleID()) &&
            ...
            console.log(`Der removeanmeldungFR Command wurde von ${interaction.user.username} verwendet -- ${date}`)
        }

        const userToRemoveSubIn = interaction.options.getUser('fahrer');

        if(!(CurrentSeason.seasonData.getSubInDriversPerCommandLigaFR().includes(userToRemoveSubIn.id))){
            ...
            if(!(client.guilds.cache.get(CurrentSeason.seasonData.getDiscordID()).members.cache.get(userToRemoveSubIn.id).roles.cache.has(CurrentSeason.seasonData.getErsatzfahrerRolleIDLigaFR()))){
            ...
            collectorConfirm.on('collect', async (reaction, user) => {
                ...
                if(reaction.emoji.name == CurrentSeason.seasonData.getAnmeldeEmoji()){
                    // Driver on waitlist
                    if(CurrentSeason.seasonData.getSubPersonListLigaFR().includes(userToRemoveSubIn.id)){
                        ...
                        let tempArray = CurrentSeason.seasonData.getSubInDriversPerCommandLigaFR();
                        tempArray.splice(tempArray.indexOf(userToRemoveSubIn.id), 1);
                        CurrentSeason.seasonData.setSubInDriversPerCommandLigaFR(tempArray);
                    ...
                    let date = new Date().toLocaleString();
                    console.log(`removeanmeldungFR wurde gestartet und erfolgreich durchgeführt für ${userToRemoveSubIn.username} -- ${date}`)
                    ...
                    await interaction.channel.send(`Der Vorgang wurde erfolgreich abgebrochen`).then(() => {
                        let date = new Date().toLocaleString();
                        console.log(`removeanmeldungFR wurde gestartet und abgebrochen -- ${date}`)
                    });
                } else {
