...
module.exports = {
    ...
    async execute(client, interaction, command){
        ...
        if(!interaction.member.roles.cache.has(CurrentSeason.seasonData.getRennleiterRolleID()) &&
            ...
            console.log(`Der removeabmeldungFR Command wurde von ${interaction.user.username} verwendet -- ${date}`)
        }

        const userToRemoveWithdraw = interaction.options.getUser('fahrer');

        if(!(CurrentSeason.seasonData.getWithdrawnDriversPerCommandLigaFR().includes(userToRemoveWithdraw.id))){
            ...
            if(!(client.guilds.cache.get(CurrentSeason.seasonData.getDiscordID()).members.cache.get(userToRemoveWithdraw.id).roles.cache.has(CurrentSeason.seasonData.getStammfahrerRolleIDLigaFR()))){
            ...
            collectorConfirm.on('collect', async (reaction, user) => {
                ...
                if(reaction.emoji.name == CurrentSeason.seasonData.getAnmeldeEmoji()){
                    ...
                    let tempArray = CurrentSeason.seasonData.getWithdrawnDriversPerCommandLigaFR();
                    tempArray.splice(tempArray.indexOf(userToRemoveWithdraw.id), 1);
                    CurrentSeason.seasonData.setWithdrawnDriversPerCommandLigaFR(tempArray); 

                    let date = new Date().toLocaleString();
                    console.log(`removeabmeldungFR wurde gestartet und erfolgreich durchgeführt für ${userToRemoveWithdraw.username} -- ${date}`)
                    // Deletes message
                    ...
                    await interaction.channel.send(`Der Vorgang wurde erfolgreich abgebrochen`).then(() => {
                        let date = new Date().toLocaleString();
                        console.log(`removeabmeldungFR wurde gestartet und abgebrochen -- ${date}`)
                    });
                } else {
