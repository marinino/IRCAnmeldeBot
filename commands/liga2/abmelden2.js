...
module.exports = {
    ...
    async execute(client, interaction, command){
        ...
        if(CurrentSeason.seasonData.getWithdrawnDriversPerCommandLiga2().includes(userToRemove.id)){
            ...
            collectorConfirm.on('collect', async (reaction, user) => {
                ...
                if(reaction.emoji.name == CurrentSeason.seasonData.getAnmeldeEmoji()){
                    ...
                    if(userToRemove && 
                        client.guilds.cache.get(CurrentSeason.seasonData.getDiscordID()).members.cache.get(markedUserID).roles.cache.has(CurrentSeason.seasonData.getStammfahrerRolleIDLiga2())){                

                        CurrentSeason.methodStorage.regularDriverWithdraw(client, userToRemove, CurrentSeason.seasonData);
                        CurrentSeason.seasonData.setWithdrawnDriversPerCommandLiga2(CurrentSeason.seasonData.getWithdrawnDriversPerCommandLiga2().concat([userToRemove.id]));
