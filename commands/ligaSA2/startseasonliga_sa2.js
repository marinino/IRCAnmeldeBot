...
async function printCalendar(interaction){
    ...
    if(seasonData.getSeasonCalendarLigaSA2().length == 0){
        stringFutureRaces = `Es sind keine \n Rennen mehr geplant`
    }else{
        seasonData.getSeasonCalendarLigaSA2().forEach((element) => {
    ...
    if(seasonData.getCurrentRaceLocationLigaSA2() == null){
        stringCurrentRace = `Aktuell läuft \n kein Event`
    }else{
        stringCurrentRace = seasonData.getCurrentRaceLocationLigaSA2()
    }
    
    // Outputs past races
    var stringPastRaces = ''
    if(seasonData.getSeasonCalendarRacesDoneLigaSA2().length == 0){
        stringPastRaces = `Bisher wurde noch kein \n Event abgeschlossen`
    }else{
        seasonData.getSeasonCalendarRacesDoneLigaSA2().forEach((element) => {
...
module.exports = {
    ...
    async execute(client, interaction, command){
        ...
        if(!interaction.member.roles.cache.has(seasonData.getRennleiterRolleID()) &&
            ...
            console.log(`Der startseasonSA2 Command wurde von ${interaction.user.username} verwendet -- ${date}`)
        }

        // Sets typ of League
        seasonData.setLigatitel('Liga SA 2')
        ...
        seasonData.setSeasonCalendarLigaSA2(calendarAsArray)
