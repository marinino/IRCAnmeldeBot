...
module.exports = {
    ...
    async execute(client, interaction, command){
        ...
        if(!interaction.member.roles.cache.has(CurrentSeason.seasonData.getRennleiterRolleID()) &&
            ...
            console.log(`Der removeanmeldungSO1 Command wurde von ${interaction.user.username} verwendet -- ${date}`)
        }

        const userToRemoveSubIn = interaction.options.getUser('fahrer');

        if(!(CurrentSeason.seasonData.getSubInDriversPerCommandLigaSO1().includes(userToRemoveSubIn.id))){
            ...
            if(!(client.guilds.cache.get(CurrentSeason.seasonData.getDiscordID()).members.cache.get(userToRemoveSubIn.id).roles.cache.has(CurrentSeason.seasonData.getErsatzfahrerRolleIDLigaSO1()))){
            ...
            collectorConfirm.on('collect', async (reaction, user) => {
                ...
                if(reaction.emoji.name == CurrentSeason.seasonData.getAnmeldeEmoji()){
                    // Driver on waitlist
                    if(CurrentSeason.seasonData.getSubPersonListLigaSO1().includes(userToRemoveSubIn.id)){
                        ...
                        let tempArray = CurrentSeason.seasonData.getSubInDriversPerCommandLigaSO1();
                        tempArray.splice(tempArray.indexOf(userToRemoveSubIn.id), 1);
                        CurrentSeason.seasonData.setSubInDriversPerCommandLigaSO1(tempArray);
                    ...
                    let date = new Date().toLocaleString();
                    console.log(`removeanmeldungSO1 wurde gestartet und erfolgreich durchgeführt für ${userToRemoveSubIn.username} -- ${date}`)
                    ...
                    await interaction.channel.send(`Der Vorgang wurde erfolgreich abgebrochen`).then(() => {
                        let date = new Date().toLocaleString();
                        console.log(`removeanmeldungSO1 wurde gestartet und abgebrochen -- ${date}`)
                    });
                } else {
                    confirmMessage.reply('Es wurde mit dem falschen Emoji reagiert').then(() => {
                      let date = new Date().toLocaleString();
                      console.log(`removeanmeldungSO1 wurde gestartet aber es wurde mit dem falschen Emoji `+
