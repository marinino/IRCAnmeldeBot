...
module.exports = {
    ...
    async execute(client, interaction, command){
        ...
        if(!interaction.member.roles.cache.has(CurrentSeason.seasonData.getRennleiterRolleID()) &&
            ...
            console.log(`Der changecockpitSO1 Command wurde von ${interaction.user.username} verwendet -- ${date}`)
        }

        const driverIn = interaction.options.getUser('driver');
        const teamObject = interaction.options.getRole('team');
        const abmeldeChannel = CurrentSeason.seasonData.getAbmeldeChannelIDLigaSO1();
        ...
        var tempSubInPerCmd = CurrentSeason.seasonData.getSubInDriversPerCommandLigaSO1();
        var tempFreeCars = CurrentSeason.seasonData.getFreeCarsLigaSO1();
        var tempWithdrawnDrivers = CurrentSeason.seasonData.getWithdrawnDriversLigaSO1();
            ...
            */
                ...
                if(driverIn){
                    ...
                    if(preCmdDriver != 'nicht besetzt'){
                        ...
                        CurrentSeason.seasonData.setWithdrawnDriversLigaSO1(tempWithdrawnDrivers);

                        let dateRemove = new Date().toLocaleString();
                        console.log(`changecockpitSO1 wurde verwendet und der Fahrer` +
                    ...
                    CurrentSeason.seasonData.setSubInDriversPerCommandLigaSO1(tempSubInPerCmd);

                    let dateIn = new Date().toLocaleString();
                    console.log(`changecockpitSO1 wurde verwendet und der Fahrer ${driverIn.username} wurde erfolgreich ins Lineup genommen -- ${dateIn}`);
