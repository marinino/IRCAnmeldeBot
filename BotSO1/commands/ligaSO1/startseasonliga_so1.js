...
async function printCalendar(interaction){
    ...
    if(seasonData.getSeasonCalendarLigaSO1().length == 0){
        stringFutureRaces = `Es sind keine \n Rennen mehr geplant`
    }else{
        seasonData.getSeasonCalendarLigaSO1().forEach((element) => {
        console.log(element)
    ...
    if(seasonData.getCurrentRaceLocationLigaSO1() == null){
        stringCurrentRace = `Aktuell läuft \n kein Event`
    }else{
        stringCurrentRace = seasonData.getCurrentRaceLocationLigaSO1()
    }
    
    // Outputs past races
    var stringPastRaces = ''
    if(seasonData.getSeasonCalendarRacesDoneLigaSO1().length == 0){
        stringPastRaces = `Bisher wurde noch kein \n Event abgeschlossen`
    }else{
        seasonData.getSeasonCalendarRacesDoneLigaSO1().forEach((element) => {
...
module.exports = {
    ...
    async execute(client, interaction, command){
        ...
        if(!interaction.member.roles.cache.has(seasonData.getRennleiterRolleID()) &&
            ...
            console.log(`Der startseasonSO1 Command wurde von ${interaction.user.username} verwendet -- ${date}`)
        }

        // Sets typ of League
        seasonData.setLigatitel('Liga SO 1')
        ...
        seasonData.setSeasonCalendarLigaSO1(calendarAsArray)
