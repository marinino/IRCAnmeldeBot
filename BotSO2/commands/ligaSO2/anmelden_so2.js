...
module.exports = {
    ...
    async execute(client, interaction, command){
        ...
        if(!interaction.member.roles.cache.has(CurrentSeason.seasonData.getRennleiterRolleID()) &&
            ...
            console.log(`Der anmeldenSO2 Command wurde von ${interaction.user.username} verwendet -- ${date}`)
        }

        const userToSubIn = interaction.options.getUser('fahrer');

        var tempSubDriversInPerCommand = CurrentSeason.seasonData.getSubInDriversPerCommandLigaSO2()
        ...
        if(tempSubDriversInPerCommand.includes(userToSubIn.id)){
            ...
            collectorConfirm.on('collect', async (reaction, user) => {
                ...
                if(reaction.emoji.name == CurrentSeason.seasonData.getAnmeldeEmoji()){
                    ...
                    if(userToSubIn && 
                        client.guilds.cache.get(CurrentSeason.seasonData.getDiscordID()).members.cache.get(markedUserID).roles.cache.has(CurrentSeason.seasonData.getErsatzfahrerRolleIDLigaSO2())){                

                        CurrentSeason.methodStorage.subDriverIn(client, userToSubIn, CurrentSeason.seasonData);
                        
                        CurrentSeason.seasonData.setSubInDriversPerCommandLigaSO2(tempSubDriversInPerCommand.concat([userToSubIn.id]));
                        
                        console.log(CurrentSeason.seasonData.getSubInDriversPerCommandLigaSO2().length)
                
                        let date = new Date().toLocaleString();
                        console.log(`anmeldenSO2 wurde erfolgreich mit ${userToSubIn.username} durchgeführt -- ${date}`);

                    }else if(userToSubIn &&
                        !(client.guilds.cache.get(CurrentSeason.seasonData.getDiscordID()).members.cache.get(markedUserID).roles.cache.has(CurrentSeason.seasonData.getErsatzfahrerRolleIDLigaSO2()))){
                        
                            let date = new Date().toLocaleString();
                            console.log(`Es ist etwas schiefgelaufen beim anmeldenSO2 Command. ${userToSubIn.username} hat nicht die Rolle Ersatzfahrer Liga 1. -- ${date}`);
                       
                    }else{
                       
                        let date = new Date().toLocaleString();
                        console.log(`Bei anmeldenSO2 war der userToSubIn undefiniert oder null. -- ${date}`);
                    ...
                    await confirmMessage.reply('Der Vorgang wurde erfolgreich abgebrochen!').then(() => {
                        let date = new Date().toLocaleString();
                        console.log(`anmeldenSO2 wurde gestartet und abgebrochen -- ${date}`)
                    });
                    await confirmMessage.delete();
                } else {
                    await confirmMessage.reply('Es wurde mit dem falschen Emoji reagiert').then(() => {
                        let date = new Date().toLocaleString();
                        console.log(`anmeldenSO2 wurde gestartet und es wurde mit dem falschen Emoji reagiert -- ${date}`)
                        })
