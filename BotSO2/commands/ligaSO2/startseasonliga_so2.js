...
async function printCalendar(interaction){
    ...
    if(seasonData.getSeasonCalendarLigaSO2().length == 0){
        stringFutureRaces = `Es sind keine \n Rennen mehr geplant`
    }else{
        seasonData.getSeasonCalendarLigaSO2().forEach((element) => {
    ...
    if(seasonData.getCurrentRaceLocationLigaSO2() == null){
        stringCurrentRace = `Aktuell läuft \n kein Event`
    }else{
        stringCurrentRace = seasonData.getCurrentRaceLocationLigaSO2()
    }
    
    // Outputs past races
    var stringPastRaces = ''
    if(seasonData.getSeasonCalendarRacesDoneLigaSO2().length == 0){
        stringPastRaces = `Bisher wurde noch kein \n Event abgeschlossen`
    }else{
        seasonData.getSeasonCalendarRacesDoneLigaSO2().forEach((element) => {
...
module.exports = {
    ...
    async execute(client, interaction, command){
        ...
        if(!interaction.member.roles.cache.has(seasonData.getRennleiterRolleID()) &&
            ...
            console.log(`Der startseasonSO2 Command wurde von ${interaction.user.username} verwendet -- ${date}`)
        }

        // Sets typ of League
        seasonData.setLigatitel('Liga SO 2')
        ...
        seasonData.setSeasonCalendarLigaSO2(calendarAsArray)
