...
module.exports = {
    ...
    async execute(client, interaction, command){
        ...
        if(!interaction.member.roles.cache.has(CurrentSeason.seasonData.getRennleiterRolleID()) &&
            ...
            console.log(`Der abmeldenSO2 Command wurde von ${interaction.user.username} verwendet -- ${date}`)
        }

        const userToRemove = interaction.options.getUser('fahrer');
        var tempWithdrawnDriversPerCommand = CurrentSeason.seasonData.getWithdrawnDriversPerCommandLigaSO2();
        ...
        if(tempWithdrawnDriversPerCommand.includes(userToRemove.id)){
            ...
            collectorConfirm.on('collect', async (reaction, user) => {
                ...
                if(reaction.emoji.name == CurrentSeason.seasonData.getAnmeldeEmoji()){
                    ...
                    if(userToRemove && 
                        client.guilds.cache.get(CurrentSeason.seasonData.getDiscordID()).members.cache.get(markedUserID).roles.cache.has(CurrentSeason.seasonData.getStammfahrerRolleIDLigaSO2())){                

                        CurrentSeason.methodStorage.regularDriverWithdraw(client, userToRemove, CurrentSeason.seasonData);
                        CurrentSeason.seasonData.setWithdrawnDriversPerCommandLigaSO2(tempWithdrawnDriversPerCommand.concat([userToRemove.id]));

                        let date = new Date().toLocaleString();
                        console.log(`abmeldenSO2 wurde erfolgreich mit ${userToRemove.username} durchgeführt -- ${date}`);
                        ...
                        !(client.guilds.cache.get(CurrentSeason.seasonData.getDiscordID()).members.cache.get(markedUserID).roles.cache.has(CurrentSeason.seasonData.getStammfahrerRolleIDLigaSO2()))){
                            ...
                            let date = new Date().toLocaleString();
                            console.log(`Es ist etwas schiefgelaufen beim abmeldenSO2 Command. ${userToRemove.username} hat nicht die Rolle Stammfahrer Liga 1. -- ${date}`);
                       
                    }else{
                       
                        let date = new Date().toLocaleString();
                        console.log(`Bei abmeldenSO2 war der userToRemove undefiniert oder null. -- ${date}`);
                    ...
                    await confirmMessage.reply('Der Vorgang wurde erfolgreich abgebrochen!').then(() => {
                        let date = new Date().toLocaleString();
                        console.log(`abmeldenSO2 wurde gestartet und abgebrochen -- ${date}`)
                    });
                    await confirmMessage.delete();
                } else {
                    await confirmMessage.reply('Es wurde mit dem falschen Emoji reagiert').then(() => {
                        let date = new Date().toLocaleString();
                        console.log(`abmeldenSO2 wurde gestartet und es wurde mit dem falschen Emoji reagiert -- ${date}`)
                        })
